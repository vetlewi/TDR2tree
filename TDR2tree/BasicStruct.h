...
struct Event {
    ...
    inline void AddRing(const int16_t &id, const double &energy, const int64_t &course, const double &fine)
    {
        if (ring_mult < NUM_MAX){
            ringID[ring_mult] = id;
            ring_energy[ring_mult] = energy;
            ring_t_course[ring_mult] = course;
            ring_t_fine[ring_mult++] = fine;
        }
    }

    inline void AddSect(const int16_t &id, const double &energy, const int64_t &course, const double &fine)
    {
        if (sect_mult < NUM_MAX){
            sectID[sect_mult] = id;
            sect_energy[sect_mult] = energy;
            sect_t_course[sect_mult] = course;
            sect_t_fine[sect_mult++] = fine;
        }
    }

    inline void AddBack(const int16_t &id, const double &energy, const int64_t &course, const double &fine)
    {
        if (back_mult < NUM_MAX){
            backID[back_mult] = id;
            back_energy[back_mult] = energy;
            back_t_course[back_mult] = course;
            back_t_fine[back_mult++] = fine;
        }
    }

    inline void AddLabrL(const int16_t &id, const double &energy, const int64_t &course, const double &fine)
    {
        if (labrL_mult < NUM_MAX){
            labrLID[labrL_mult] = id;
            labrL_energy[labrL_mult] = energy;
            labrL_t_course[labrL_mult] = course;
            labrL_t_fine[labrL_mult++] = fine;
        }
    }

    inline void AddLabrS(const int16_t &id, const double &energy, const int64_t &course, const double &fine)
    {
        if (labrS_mult < NUM_MAX){
            labrSID[labrS_mult] = id;
            labrS_energy[labrS_mult] = energy;
            labrS_t_course[labrS_mult] = course;
            labrS_t_fine[labrS_mult++] = fine;
        }
    }

    inline void AddClover(const int16_t &id, const double &energy, const int64_t &course, const double &fine)
    {
        if (clover_mult < NUM_MAX){
            cloverID[clover_mult] = id;
            clover_energy[clover_mult] = energy;
            clover_t_course[clover_mult] = course;
